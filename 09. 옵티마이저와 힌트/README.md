...
MySQL 옵티마이저는 실행 계획을 수립할 때 통계 정보와 옵티마이저 옵션을 함께 고려해 최적의 계획을 선택한다.

옵티마이저 스위치는 MySQL 5.5부터 도입되었고 고급 최적화 기능의 사용 여부를 제어하는 용도로 사용된다.
...
default, on, off 중 하나로 설정 가능하다.

## **9.3.1.1 MRR과 배치 키 액세스**

### **기본 조인 방식 문제**

기존 Nested Loop Join은 드라이빙 테이블에서 한 건 읽을 때마다 드리븐 테이블을 즉시 랜덤 접근한다.

스토리지 엔진 입장에서는 최적화 여지가 거의 없다.
...
- MRR을 활용한 조인 방식
- 기본적으로 off
- 정렬 비용 때문에 항상 유리하지는 않다.

## **9.3.1.2 블록 네스티드 루프 조인(block_nested_loop)**

조인 버퍼를 사용해 드라이빙 테이블 결과를 모아두고 드리븐 테이블을 한 번에 처리한다.

- join_buffer_size에 의존
- 실행 계획 Extra에 Using join buffer 표시
- 조인 순서와 결과 정렬 순서가 달라질 수 있음

MySQL 8.0.20 이후 해시 조인으로 대체되었다.

## **9.3.1.3 인덱스 컨디션 푸시다운(index_condition_pushdown)**

### **도입 배경**

- 인덱스로 범위를 줄일 수 없는 조건이 엔진까지 전달되지 않던 문제
...
InnoDB 세컨더리 인덱스는 리프 노드에 자동으로 프라이머리 키를 포함한다.

옵티마이저가 이를 인식해 복합 인덱스처럼 활용할 수 있게 한 기능이다.

## **9.3.1.5 인덱스 머지**

하나의 테이블에서 여러 인덱스를 동시에 사용하는 실행 계획이다.

단일 인덱스로 충분할 때는 선택되지 않는다.
...
각 인덱스를 따로 사용하는 것보다 읽어야 할 레코드 수가 줄어들 때만 선택됨
...
- 서브쿼리 결과가 유니크하고 1건일 때 유리

## **9.3.1.11 퍼스트 매치**

- EXISTS 형태의 세미 조인
- 첫 번째 일치 레코드 발견 시 즉시 중단
- Short circuit 방식

Extra에 FirstMatch 표시

GROUP BY 서브쿼리에는 사용 불가

## **9.3.1.12 루스 스캔**

- 서브쿼리에서 루스 인덱스 스캔 활용
- GROUP BY 최적화와 유사한 방식

Extra에 LooseScan 표시

## **9.3.1.13 구체화**

서브쿼리 결과를 임시 테이블로 만들어 조인하는 방식

- MATERIALIZED 표시
- 내부 임시 테이블 생성
- GROUP BY 포함 서브쿼리도 가능

## **9.3.1.14 중복 제거**

Duplicate Weedout

- 세미 조인을 내부적으로 JOIN + GROUP BY 형태로 처리
- 임시 테이블 사용
- Start temporary
- End temporary 로 구간 표시

## **9.3.1.15 컨디션 팬아웃 필터**

옵티마이저가 조건별 필터링 비율을 더 정확히 계산하도록 개선

filtered 컬럼 값 변화로 확인 가능
...
인덱스를 삭제하지 않고 옵티마이저에서만 제외

- 기본 off
- use_invisible_indexes 옵션 필요
...
Nested Loop Join이 부적합할 때 대체 전략으로 사용

메모리 초과 시 Grace hash join 방식으로 디스크 사용

## **9.3.1.20 인덱스 정렬 선호**

ORDER BY나 GROUP BY를 인덱스로 처리하려는 경향을 제어

옵티마이저가 잘못 판단하는 경우를 대비해 prefer_ordering_index 옵션 제공
